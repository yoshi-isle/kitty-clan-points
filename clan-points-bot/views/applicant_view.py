...
class QuestionModal(discord.ui.Modal, title="Clan Application"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            message = await interaction.channel.fetch_message(applicant_record['application_embed_message_id'])
            # Edit the application
            embed = message.embeds[0] if message.embeds else None
            if embed:
                embed.set_field_at(0, name="RuneScape Name(s)", value=f"```{self.question1.value}```", inline=False)
                embed.set_field_at(1, name="How long have you been playing?", value=f"```{self.question2.value}```", inline=False)
                embed.set_field_at(2, name="Why do you want to join our clan?", value=f"```{self.question3.value}```", inline=False)
...
class ApplicantView(discord.ui.View):
    ...
    @discord.ui.button(label="🗒️Answer questions", style=discord.ButtonStyle.primary, custom_id="answer_questions")
    ...
    @discord.ui.button(label="✅(Admin) Approve", style=discord.ButtonStyle.secondary, custom_id="approve_member")
